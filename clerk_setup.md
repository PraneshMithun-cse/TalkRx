...
This setup is linked to the Clerk application `app_3IPea4dC7MKjQXXSOoijC52bYSV`. Always
pass `--app app_3IPea4dC7MKjQXXSOoijC52bYSV` to `clerk init` so the project links to
the right Clerk app.

If this is an existing project, run:

```bash
clerk init --app app_3IPea4dC7MKjQXXSOoijC52bYSV
```
...
```bash
clerk init --framework <framework> --pm <package-manager> --app app_3IPea4dC7MKjQXXSOoijC52bYSV
```
