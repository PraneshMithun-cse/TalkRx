...
export function serializePatient(p: PatientWithRelations): PatientProfile {
  ...
  // Deliberately no fallback here: fabricating a "128/82, 134 mg/dL" reading for a
  // patient with no vitals on record would feed a fake number straight into the
  // Doctor Dashboard's clinical classifier (classifyVitals) and could label someone
  // "Elevated Blood Pressure" who's never had a real reading taken. Consumers that
  // want a display placeholder (e.g. pre-filling an edit form) do that at the UI
  // layer instead, where it's clearly a form default rather than a patient value.
  const vitals = rawSummary?.vitals as PatientProfile["vitals"] | undefined;
